...
iris_data.X = readmatrix('iris.csv','Range','A2:D151');
...
p3 = zeros(1,size(X,1));
for i=1:150
    p1(i) = (1/50)*sum(gaussianKernel(X(i,:), X(1:50,:), spread));
    p2(i) = (1/50)*sum(gaussianKernel(X(i,:), X(51:100,:), spread));
    p3(i) = (1/50)*sum(gaussianKernel(X(i,:), X(101:150,:), spread));
end
...
[indx_trn,indx_tst] = crossval(length(X),cross_Val);
...
for k = 1:cross_Val
    ...
    for i=1:length(test)
        p1(i) = 1/length(train1)*sum(gaussianKernel(test(i,:), train1, spread));
    end
    ...
    for i=1:length(test)
        p2(i) = 1/length(train2)*sum(gaussianKernel(test(i,:), train2, spread));
    end
    ...
    for i=1:length(test)
        p3(i) = 1/length(train3)*sum(gaussianKernel(test(i,:), train3, spread));
    end
    ...
    [a1p1, a2p1, a3p1, a1p2, a2p2, a3p2, a1p3, a2p3, a3p3] = ...
                                    confusion_matrix_3Class(ypred', yTest);
    ...
    fprintf('                           Actual                      \n')
    fprintf('P|-----------------------------------------------------\n')
    fprintf('r|             Setosa   Versicolor   Viginica          \n')
    fprintf('e|                                                     \n')
...
for i=1:length(ax)
    p1(i) = (1/50)*sum(gaussianKernel(ax(i), X(1:50,2), spread));
    p2(i) = (1/50)*sum(gaussianKernel(ax(i), X(51:100,2), spread));
    p3(i) = (1/50)*sum(gaussianKernel(ax(i), X(101:150,2), spread));
end
...
hold on;plot(X(51:100,2),zeros(1,50),'go','MarkerSize',8,'LineWidth',1.5)
...
p3=zeros(lngtAxy,1);
for i=1:lngtAxy
    p1(i) = (1/50)*sum(gaussianKernel(Axy(i,:), X(1:50,:), spread));
    p2(i) = (1/50)*sum(gaussianKernel(Axy(i,:), X(51:100,:), spread));
    p3(i) = (1/50)*sum(gaussianKernel(Axy(i,:), X(101:150,:), spread));
end
...
hold on;plot(X(inx2,1),X(inx2,2),'go','LineWidth',2,'MarkerSize',7)
